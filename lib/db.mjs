...
/**
 * Neon is serverless: a cold compute, a sleeping laptop radio, or an ordinary
 * blip answers with `fetch failed` rather than a Postgres error. Retrying three
 * times over roughly ten seconds turns that into a pause instead of a lost
 * night's reading.
 */
async function withRetry(label, fn, attempts = 3) {
  for (let i = 1; ; i++) {
    try {
      ...
      console.error(`  ${label}: attempt ${i} failed (${e.message}) — retrying in ${wait}ms`);
...
export async function upsertDays(days, host) {
  for (const day of days) {
    await withRetry(`neon ${day.date}`, () => upsertDay(day, host));
  }
  return days.length;
}

export const retrying = withRetry;
